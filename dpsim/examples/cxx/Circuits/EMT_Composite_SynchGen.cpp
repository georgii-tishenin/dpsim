...
void CompositeSynchGen() {
  ...
  v3->setParameters(Complex(-5.0, 5.0*sqrt(3.0)), 50);
    ...
    auto R0 = Resistor::make("R0", Logger::Level::debug);  // zero sequence resistor
    R0->setParameters(1e3);

    // d-axis components
    auto Rsd = Resistor::make("Rsd", Logger::Level::debug);  //stator winding resistor d-axis
    Rsd->setParameters(0.00311);
    auto Ls_sigma_d = Inductor::make("Ls_sigma", Logger::Level::debug); // leakage stator inductance d-axis
    Ls_sigma_d->setParameters(4.9553e-4);  
    auto Lmd = Inductor::make("Lmd", Logger::Level::debug);  // mutual inductance d-axis
    Lmd->setParameters(0.00548);
    auto L1d = Inductor::make("L1_d", Logger::Level::debug);  // d-axis damping winding inductance
    L1d->setParameters(5.659e-4);
    auto R1d = Resistor::make("R1d", Logger::Level::debug);  // d-axis damping winding resistor
    R1d->setParameters(0.02947);
    auto Lf_d = Inductor::make("Lf_d", Logger::Level::debug);  // Field winding inductance
    Lf_d->setParameters(5.451e-4);
    auto Rf_d = Resistor::make("Rf_d", Logger::Level::debug);  // Field winding resistor
    Rf_d->setParameters(6.227e-4);
    auto Vf_d = VoltageSource::make("Vf_d", Logger::Level::debug);  // Generator excitation voltage
    ...
    // q-axis components
    auto Rsq = Resistor::make("Rsq", Logger::Level::debug);  //stator winding resistor q-axis
    Rsq->setParameters(0.00311);
    auto Ls_sigma_q = Inductor::make("Lssigma_q", Logger::Level::debug); // leakage stator inductance q-axis
    Ls_sigma_q->setParameters(4.9553e-4);  
    auto Lmq = Inductor::make("Lmq", Logger::Level::debug);  // mutual inductance q-axis
    Lmq->setParameters(0.00532);
    auto L1q = Inductor::make("L1q", Logger::Level::debug);  // q-axis 1st damping winding inductance
    L1q->setParameters(0.00239);
    auto R1q = Resistor::make("R1q", Logger::Level::debug);  // q-axis 1st damping winding resistor
    R1q->setParameters(0.00642);
    auto L2q = Inductor::make("L2q", Logger::Level::debug);  // q-axis 2nd damping winding inductance
    L2q->setParameters(4.129e-4);
    auto R2q = Resistor::make("R2q", Logger::Level::debug);  // q-axis 2nd damping winding resistor
    R2q->setParameters(0.02458);


    auto ElectroMechanicalConverter_d = IdealTransformerVariableRatio::make("ElectroMechanicalConverter_d", Logger::Level::debug);
    ElectroMechanicalConverter_d->setStatorInductor(Ls_sigma_q);
    ElectroMechanicalConverter_d->setTimeStep(timeStep);
    //ElectroMechanicalConverter_d->setVoltageNegative(true); // Set the voltage to be negative for the speed term

    auto ElectroMechanicalConverter_q = IdealTransformerVariableRatio::make("ElectroMechanicalConverter_q", Logger::Level::debug);
    ElectroMechanicalConverter_q->setStatorInductor(Ls_sigma_d);
    ElectroMechanicalConverter_q->setTimeStep(timeStep);

    auto ParkTrafo = ParkTransformer::make("ParkTransformer", Logger::Level::debug);
    ParkTrafo->isOmegaConstant(true); // Set the Park transformer to use a constant angular frequency
    ParkTrafo->setParameters(2*M_PI*50, 0.0); // Set the angular frequency and initial angle

    //Mechanical components
    auto InertiaMoment = InertiaMoment::make("InertiaMoment", Logger::Level::debug);
    ...
    auto ConstantOmegaSource = VoltageSource::make("ConstantOmegaSource", Logger::Level::debug);
    ConstantOmegaSource->setParameters(Complex(2*M_PI*50, 0.0)); // Set the constant angular frequency source
    ...
    // 0 axis circuit
    R0->connect({n17, SimNode::GND}); // zero sequence resistor connects to node n6
  ...
  SystemTopology system(50, 
    SystemNodeList{n1, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11, n12, n13, n14, n15, n16, n17, n18},
    SystemComponentList{R1d, Rsd, Ls_sigma_d, Lmd, L1d, R1q, Rsq, Ls_sigma_q, Lmq, L1q, L2q, R2q,
                        ElectroMechanicalConverter_d, ElectroMechanicalConverter_q, ParkTrafo, Vf_d, Rf_d, InertiaMoment, v1, v2, v3, R0, Lf_d, LoadTorque, ConstantOmegaSource});
  ...
  logger->logAttribute("Ratio_d", ElectroMechanicalConverter_d->attribute("Ratio"));
  logger->logAttribute("Ratio_q", ElectroMechanicalConverter_q->attribute("Ratio"));
...
//  cmake --build . --target EMT_Composite_SynchGen -- -j10
...
EMT::Ph3::SynchronGeneratorVBR::SynchronGeneratorVBR(String uid, String name,
...
void EMT::Ph3::SynchronGeneratorVBR::setBaseAndOperationalPerUnitParameters(
  ...
  Base::SynchronGenerator::setBaseAndOperationalPerUnitParameters(
      nomPower, nomVolt, nomFreq, poleNumber, nomFieldCur, Rs, Ld, Lq, Ld_t,
      Lq_t, Ld_s, Lq_s, Ll, Td0_t, Tq0_t, Td0_s, Tq0_s, inertia);
  ...
  SPDLOG_LOGGER_INFO(mSLog,
                     ...
                     "Ld_t: {:e}\nLq_t: {:e}\nLd_s: {:e}\nLq_s: {:e}\n"
                     "Td0_t: {:e}\nTq0_t: {:e}\nTd0_s: {:e}\nTq0_s: {:e}\n",
                     poleNumber, inertia, Rs, Ld, Lq, Ll, Ld_t, Lq_t, Ld_s,
                     Lq_s, Td0_t, Tq0_t, Td0_s, Tq0_s);

  SPDLOG_LOGGER_INFO(
      mSLog,
      "Set fundamental parameters in per unit: \n"
      "Rs: {:e}\nLl: {:e}\nLmd: {:e}\nLmq: {:e}\nRfd: {:e}\nLlfd: {:e}\nRkd: "
      "{:e}\n"
      "Llkd: {:e}\nRkq1: {:e}\nLlkq1: {:e}\nRkq2: {:e}\nLlkq2: {:e}\n",
      **mRs, **mLl, mLmd, mLmq, mRfd, mLlfd, mRkd, mLlkd, mRkq1, mLlkq1, mRkq2,
      mLlkq2);
}
...
void EMT::Ph3::SynchronGeneratorVBR::setBaseAndFundamentalPerUnitParameters(
  ...
  SPDLOG_LOGGER_INFO(mSLog,
                     ...
                     "Llkd: {:e}\nRkq1: {:e}\nLlkq1: {:e}\nRkq2: {:e}\nLlkq2: "
                     "{:e}\ninertia: {:e}",
                     nomPower, nomVolt, nomFreq, poleNumber, nomFieldCur, Rs,
                     Ll, Lmd, Lmq, Rfd, Llfd, Rkd, Llkd, Rkq1, Llkq1, Rkq2,
                     Llkq2, inertia);
}

void EMT::Ph3::SynchronGeneratorVBR::setInitialValues(Real initActivePower,
...
void EMT::Ph3::SynchronGeneratorVBR::initializeFromNodesAndTerminals(
    Real frequency) {
  if (!mInitialValuesSet) {
    ...
    this->setInitialValues(activePower, reactivePower, voltMagnitude,
                           Math::phase(initialSingleVoltage(0)), activePower);
    ...
    SPDLOG_LOGGER_INFO(mSLog, "Initial values already set, skipping "
                              "initializeFromNodesAndTerminals.");
  }
  mSLog->flush();
}

void EMT::Ph3::SynchronGeneratorVBR::mnaCompInitialize(
  ...
  for (UInt phase1Idx = 0; phase1Idx < 3; ++phase1Idx)
    for (UInt phase2Idx = 0; phase2Idx < 3; ++phase2Idx)
      mVariableSystemMatrixEntries.push_back(std::make_pair<UInt, UInt>(
          matrixNodeIndex(0, phase1Idx), matrixNodeIndex(0, phase2Idx)));
  ...
  mResistanceMat << **mRs, 0, 0, 0, **mRs, 0, 0, 0, **mRs;
  ...
  **mMechTorque = -**mMechTorque;
  ...
  mPsikq1kq2 << mPsikq1, mPsikq2;
  mPsifdkd << mPsifd, mPsikd;
  ...
  SPDLOG_LOGGER_INFO(mSLog, "Initialize right side vector of size {}",
                     leftVector->get().rows());
  SPDLOG_LOGGER_INFO(
...
void EMT::Ph3::SynchronGeneratorVBR::mnaCompAddPreStepDependencies(
...
void EMT::Ph3::SynchronGeneratorVBR::stepInPerUnit() {
  ...
  **mOmMech = **mOmMech + mTimeStep * (1. / (2. * **mInertia) *
  ...
  mThetaMech = mThetaMech + mTimeStep * (**mOmMech * mBase_OmMech);            // Why use euler for estimation and not implicit trapezoidal?
  ...
  mEsh_vbr =
      (mResistanceMat - (2 / (mTimeStep * mBase_OmElec)) * mDInductanceMat) *
          mIabc +
      mDVabc - mVabc;
  ...
  R_eq_vbr =
      mResistanceMat + (2 / (mTimeStep * mBase_OmElec)) * mDInductanceMat + K;
...
void EMT::Ph3::SynchronGeneratorVBR::mnaCompPostStep(
    Real time, Int timeStepCount, Attribute<Matrix>::Ptr &leftVector) {
  if (terminalNotGrounded(0)) {
    mVa = Math::realFromVectorElement(*leftVector, matrixNodeIndex(0, 0)) /
          mBase_V;
    mVb = Math::realFromVectorElement(*leftVector, matrixNodeIndex(0, 1)) /
          mBase_V;
    mVc = Math::realFromVectorElement(*leftVector, matrixNodeIndex(0, 2)) /
          mBase_V;
  } else {
    mVa = 0;
    mVb = 0;
...
void EMT::Ph3::SynchronGeneratorVBR::mnaCompAddPostStepDependencies(
    AttributeBase::List &prevStepDependencies,
    AttributeBase::List &attributeDependencies,
    AttributeBase::List &modifiedAttributes,
...
void EMT::Ph3::SynchronGeneratorVBR::CalculateL() {
  mDInductanceMat << **mLl + mLa - mLb * cos(2 * mThetaMech),
      -mLa / 2 - mLb * cos(2 * mThetaMech - 2 * PI / 3),
      -mLa / 2 - mLb * cos(2 * mThetaMech + 2 * PI / 3),
      -mLa / 2 - mLb * cos(2 * mThetaMech - 2 * PI / 3),
      **mLl + mLa - mLb * cos(2 * mThetaMech - 4 * PI / 3),
      -mLa / 2 - mLb * cos(2 * mThetaMech),
      -mLa / 2 - mLb * cos(2 * mThetaMech + 2 * PI / 3),
      -mLa / 2 - mLb * cos(2 * mThetaMech),
      **mLl + mLa - mLb * cos(2 * mThetaMech + 4 * PI / 3);
}
...
void EMT::Ph3::SynchronGeneratorVBR::CalculateAuxiliarConstants(Real dt) {
  ...
  c23 = mDLmd * mRfd / (mLlfd * mLlfd) * (mDLmd / mLlfd - 1) +
        mDLmd * mDLmd * mRkd / (mLlkd * mLlkd * mLlfd);
  c24 = mDLmd * mRkd / (mLlkd * mLlkd) * (mDLmd / mLlkd - 1) +
        mDLmd * mDLmd * mRfd / (mLlfd * mLlfd * mLlkd);
  ...
  if (mNumDampingWindings == 2) {
    ...
    c11 = mDLmq * mRkq1 / (mLlkq1 * mLlkq1) * (mDLmq / mLlkq1 - 1) +
          mDLmq * mDLmq * mRkq2 / (mLlkq2 * mLlkq2 * mLlkq1);
    c12 = mDLmq * mRkq2 / (mLlkq2 * mLlkq2) * (mDLmq / mLlkq2 - 1) +
          mDLmq * mDLmq * mRkq1 / (mLlkq1 * mLlkq1 * mLlkq2);
    c15 =
        (mRkq1 / (mLlkq1 * mLlkq1) + mRkq2 / (mLlkq2 * mLlkq2)) * mDLmq * mDLmq;

    Ea << 2 - dt * b11, -dt * b12, -dt * b21, 2 - dt * b22;
    E1b << dt * b13, dt * b23;
    ...
    E2b << 2 + dt * b11, dt * b12, dt * b21, 2 + dt * b22;
    E2 = Ea_inv * E2b;
  } else {
    c11 = mDLmq * mRkq1 / (mLlkq1 * mLlkq1) * (mDLmq / mLlkq1 - 1);
  ...
  Fa << 2 - dt * b31, -dt * b32, -dt * b41, 2 - dt * b42;

  F1b << dt * b33, dt * b43;
  F1 = Fa.inverse() * F1b;

  F2b << 2 + dt * b31, dt * b32, dt * b41, 2 + dt * b42;
...
void EMT::Ph3::SynchronGeneratorVBR::CalculateAuxiliarVariables() {
  ...
  if (mNumDampingWindings == 2) {
    ...
    K1a << c11, c12, c21_omega, c22_omega;
    K1b << c15, 0;
    K1 = K1a * E1 + K1b;
  } else {
    c21_omega = -**mOmMech * mDLmq / mLlkq1;
  ...
  K2a << c13_omega, c14_omega, c23, c24;
  K2b << 0, c25;
  K2 = K2a * F1 + K2b;

  K << K1, K2, Matrix::Zero(2, 1), 0, 0, 0;

  mKrs_teta << 2. / 3. * cos(mThetaMech),
      2. / 3. * cos(mThetaMech - 2. * M_PI / 3.),
      2. / 3. * cos(mThetaMech + 2. * M_PI / 3.), 2. / 3. * sin(mThetaMech),
      2. / 3. * sin(mThetaMech - 2. * M_PI / 3.),
      2. / 3. * sin(mThetaMech + 2. * M_PI / 3.), 1. / 3., 1. / 3., 1. / 3.;

  mKrs_teta_inv << cos(mThetaMech), sin(mThetaMech), 1.,
...
Matrix EMT::Ph3::SynchronGeneratorVBR::parkTransform(Real theta, Real a, Real b,
                                                     Real c) {
  ...
  q = 2. / 3. * cos(theta) * a + 2. / 3. * cos(theta - 2. * M_PI / 3.) * b +
      2. / 3. * cos(theta + 2. * M_PI / 3.) * c;
  d = 2. / 3. * sin(theta) * a + 2. / 3. * sin(theta - 2. * M_PI / 3.) * b +
      2. / 3. * sin(theta + 2. * M_PI / 3.) * c;
...
Matrix EMT::Ph3::SynchronGeneratorVBR::inverseParkTransform(Real theta, Real q,
                                                            Real d, Real zero) {
  ...
  b = cos(theta - 2. * M_PI / 3.) * q + sin(theta - 2. * M_PI / 3.) * d +
      1. * zero;
  c = cos(theta + 2. * M_PI / 3.) * q + sin(theta + 2. * M_PI / 3.) * d +
      1. * zero;

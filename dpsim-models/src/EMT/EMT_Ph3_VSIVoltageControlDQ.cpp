...
EMT::Ph3::VSIVoltageControlDQ::VSIVoltageControlDQ(String uid, String name, Logger::Level logLevel, Bool withTrafo) :
	...
	**mIntfCurrent = Matrix::Zero(3, 1);
	...
	// Pre-step of controlled source handled explicitly in mnaParentPreStep
	...
	// Control sub components (NO DROOP)
...
// Keep signature for compatibility; droop parameters ignored.
void EMT::Ph3::VSIVoltageControlDQ::setControllerParameters(Real Kp_voltageCtrl, Real Ki_voltageCtrl,
...
void EMT::Ph3::VSIVoltageControlDQ::setTransformerParameters(Real nomVoltageEnd1, Real nomVoltageEnd2, Real ratedPower,
	...
	if (mWithConnectionTransformer)
		mConnectionTransformer->setParameters(
			mTransformerNominalVoltageEnd1, mTransformerNominalVoltageEnd2, mTransformerRatedPower,
...
void EMT::Ph3::VSIVoltageControlDQ::initializeFromNodesAndTerminals(Real frequency) {
	// --- EXACT original electrical initialization ---
	...
	if (mWithConnectionTransformer) {
		filterInterfaceInitialVoltage =
			(intfVoltageComplex - Complex(mTransformerResistance, mTransformerInductance * **mOmegaN) * intfCurrentComplex) /
	...
	// Series-branch (inverter-side) current through Rf/Lf is (I_interface - I_cap)
	MatrixComp iSeriesInit = (filterInterfaceInitialCurrent - icfInit);

	MatrixComp vfInit = vcInit - iSeriesInit * Complex(0., 2. * PI * frequency * mLf);
	MatrixComp vsInit = vfInit - iSeriesInit * Complex(mRf, 0);

	// Set initial voltages of internal nodes (RMS3ph)
	...
	**mElecPassivePower = ( 3./2. * intfVoltageComplex(0,0) * std::conj( - intfCurrentComplex(0,0)) ).imag();
	...
	// Initialize controlled source EXACTLY like original baseline
	...
	// --- Control init: consistent with runtime sensing (NO DROOP) ---
	{
		Matrix vcdq, ircdq;

		// Align dq-frame to capacitor voltage (consistent regardless of transformer)
		Real theta = std::arg(vcInit(0,0));

		// Controller senses:
		//  - Vc at capacitor node (node2)
		//  - inverter-side current (series branch) like runtime uses -ResistorF current
		vcdq  = parkTransformPowerInvariant(theta, vcInit.real());
		ircdq = parkTransformPowerInvariant(theta, -1.0 * iSeriesInit.real());
		...
		**mIrcq = ircdq(1, 0);
		...
		// Optional: keep this as "controller-side" power estimate (not PCC power)
		**mElecActivePower = **mIrcd * **mVcd + **mIrcq * **mVcq;

		// VCO: input omegaNom, state/output theta
...
void EMT::Ph3::VSIVoltageControlDQ::addControlPreStepDependencies(AttributeBase::List &prevStepDependencies,
...
void EMT::Ph3::VSIVoltageControlDQ::addControlStepDependencies(AttributeBase::List &prevStepDependencies,
                                                               AttributeBase::List &attributeDependencies,
                                                               AttributeBase::List &modifiedAttributes) {
...
void EMT::Ph3::VSIVoltageControlDQ::controlStep(Real time, Int timeStepCount) {
    ...
    // 1) Advance VCO first -> theta at current step
    mVCO->signalStep(time, timeStepCount);
    Real theta = (**mVCO->mOutputCurr)(0,0);

    // 2) Measure using the same theta (time-aligned)
    Matrix vcdq  = parkTransformPowerInvariant(theta, **mVirtualNodes[2]->mVoltage);
    Matrix ircdq = parkTransformPowerInvariant(theta, - **mSubResistorF->mIntfCurrent);

    Matrix intfVoltageDQ = parkTransformPowerInvariant(theta, **mIntfVoltage);
    Matrix intfCurrentDQ = parkTransformPowerInvariant(theta, **mIntfCurrent);

    **mElecActivePower  = -1.0 * (intfVoltageDQ(0,0)*intfCurrentDQ(0,0) + intfVoltageDQ(1,0)*intfCurrentDQ(1,0));
    **mElecPassivePower = -1.0 * (intfVoltageDQ(1,0)*intfCurrentDQ(0,0) - intfVoltageDQ(0,0)*intfCurrentDQ(1,0));
    ...
    **mIrcq = ircdq(1,0);
    ...
    // 4) Synthesize using *current* theta and *current* dq voltage ref
    **mVsref = inverseParkTransformPowerInvariant(theta, mVoltageControllerVSI->mOutputCurr->get());
}


void EMT::Ph3::VSIVoltageControlDQ::mnaParentAddPreStepDependencies(AttributeBase::List &prevStepDependencies,
                                                                    AttributeBase::List &attributeDependencies,
                                                                    AttributeBase::List &modifiedAttributes) {
...
void EMT::Ph3::VSIVoltageControlDQ::mnaParentAddPostStepDependencies(AttributeBase::List &,
                                                                     AttributeBase::List &attributeDependencies,
                                                                     AttributeBase::List &modifiedAttributes,
...
void EMT::Ph3::VSIVoltageControlDQ::mnaParentPostStep(Real, Int, Attribute<Matrix>::Ptr &leftVector) {
	mnaCompUpdateCurrent(**leftVector);
	mnaCompUpdateVoltage(**leftVector);
}

void EMT::Ph3::VSIVoltageControlDQ::mnaCompUpdateCurrent(const Matrix&) {
	if (mWithConnectionTransformer)

...
DP::Ph1::ProfileVoltageSource::ProfileVoltageSource(
    String uid, String name, std::filesystem::path sourceFile,
    Logger::Level logLevel)
    : MNASimPowerComp<Complex>(uid, name, true, true, logLevel),
      mVoltage(mAttributes->createDynamic<Complex>("V")),
      mSourceFile(sourceFile), mSourceIndex(0), mSamples() {
...
void DP::Ph1::ProfileVoltageSource::readFromFile() {
  ...
  if (!file.is_open()) {
    throw SystemError(
        "ProfileVoltageSource::ProfileVoltageSource could not open file " +
        mSourceFile.string());
  }
...
void DP::Ph1::ProfileVoltageSource::initializeFromNodesAndTerminals(
    Real frequency) {
...
void DP::Ph1::ProfileVoltageSource::mnaCompAddPreStepDependencies(
...
void DP::Ph1::ProfileVoltageSource::mnaCompAddPostStepDependencies(
    AttributeBase::List &prevStepDependencies,
    AttributeBase::List &attributeDependencies,
    AttributeBase::List &modifiedAttributes,
...
void DP::Ph1::ProfileVoltageSource::mnaCompInitialize(
...
void DP::Ph1::ProfileVoltageSource::mnaCompInitializeHarm(
...
void DP::Ph1::ProfileVoltageSource::mnaCompApplySystemMatrixStamp(
...
void DP::Ph1::ProfileVoltageSource::mnaCompApplySystemMatrixStampHarm(
...
void DP::Ph1::ProfileVoltageSource::mnaCompApplyRightSideVectorStamp(
    Matrix &rightVector) {
...
void DP::Ph1::ProfileVoltageSource::mnaCompApplyRightSideVectorStampHarm(
    Matrix &rightVector) {
  for (UInt freq = 0; freq < mNumFreqs; freq++) {
    // TODO: Is this correct with two nodes not gnd?
    Math::setVectorElement(rightVector, mVirtualNodes[0]->matrixNodeIndex(),
...
void DP::Ph1::ProfileVoltageSource::MnaPreStepHarm::execute(Real time,
                                                            Int timeStepCount) {
...
void DP::Ph1::ProfileVoltageSource::mnaCompPostStep(
    Real time, Int timeStepCount, Attribute<Matrix>::Ptr &leftVector) {
  mnaCompUpdateCurrent(**leftVector);
}

void DP::Ph1::ProfileVoltageSource::MnaPostStepHarm::execute(
    Real time, Int timeStepCount) {
  mVoltageSource.mnaCompUpdateCurrent(**mLeftVectors[0]);
}

void DP::Ph1::ProfileVoltageSource::mnaCompUpdateCurrent(
...
void DP::Ph1::ProfileVoltageSource::daeResidual(double ttime,
                                                const double state[],

...
/// \brief Moment of inertia model
///
/// The moment of inertia is represented by a DC equivalent circuit which corresponds to one
/// iteration of the trapezoidal integration method.
/// The equivalent DC circuit is a resistance (the inverse of the damping coefficient D) in paralel with a current source (torque acting on the mass).
/// The resistance is constant for a defined time step and system
///frequency and the current source changes for each iteration.
class InertiaMoment : public MNASimPowerComp<Real>,
                      public Base::Ph1::Capacitor,
                      public SharedFactory<InertiaMoment>,
...
public:
  ...
  InertiaMoment(String uid, String name,
                Logger::Level logLevel = Logger::Level::off);
  /// Defines name and logging level
  InertiaMoment(String name, Logger::Level logLevel = Logger::Level::off)
      : InertiaMoment(name, name, logLevel) {}
  ...
  //void updateParkTransformerOmega(bool isParkTransformer) final;
};
} // namespace Ph1
} // namespace EMT
} // namespace CPS
